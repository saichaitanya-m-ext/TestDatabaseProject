CREATE FUNCTION [dbo].[ufn_HEDIS_GetPatients_LabData2]
(
...
(
	...
	SELECT pm.[PatientID], pm.[MeasureID], m.[Name] AS 'MeasureName', muom.[UOMText], pm.[MeasureValueText],
		   ...
		   cod_proc.[ProcedureName], cod_proc.[ProcedureShortDescription], pat.[UserID], u.[UserLoginName],
		   pat.[NamePrefix], pat.[FirstName], pat.[MiddleName], pat.[LastName], pat.[NameSuffix],
		   pat.[Title], pat.[PreferredName], pat.[SSN], pat.[DateOfBirth],
		   DATEDIFF(yyyy, pat.[DateOfBirth], GETDATE()) AS 'PresentAge',
	...
	INNER JOIN [Patient] pat ON pat.[PatientID] = pm.[PatientID]
