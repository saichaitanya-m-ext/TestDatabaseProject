CREATE FUNCTION [dbo].[ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_SpecialtyContionEyeExam_MP] (
...
BEGIN
  ...
  *********************************************************************************************************************************************/
	...
	SET @DateKey = CONVERT(VARCHAR(10), @AnchorDate_Year) + CONVERT(VARCHAR(10), @AnchorDate_Month) + CONVERT(VARCHAR(10), @AnchorDate_Day)
		...
		INNER JOIN ClaimProvider cp1 ON cp1.ClaimInfoID = ci.ClaimInfoID
		INNER JOIN ProviderSpecialty ps ON ps.ProviderID = cp1.ProviderID
		INNER JOIN CodeSetCMSProviderSpecialty cmssp ON cmssp.CMSProviderSpecialtyCodeID = ps.CMSProviderSpecialtyCodeID
		...
		WHERE (
			...
			AND cmssp.ProviderSpecialtyCode IN (
				'18'
				,'41'
				)
