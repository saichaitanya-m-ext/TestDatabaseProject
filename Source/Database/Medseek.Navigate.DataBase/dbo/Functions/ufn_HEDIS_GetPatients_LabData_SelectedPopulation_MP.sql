CREATE  FUNCTION [dbo].[ufn_HEDIS_GetPatients_LabData_SelectedPopulation_MP] (
...
BEGIN
	...
	INSERT INTO @OUTPUT
		...
		LEFT OUTER JOIN [dbo].[Measure] m
			ON m.[MeasureId] = pm.[MeasureId]
		LEFT OUTER JOIN [dbo].[MeasureUOM] muom
			ON muom.[MeasureUOMId] = pm.[MeasureUOMId]
		LEFT OUTER JOIN [dbo].[CodeSetLOINC] cod_loinc
			ON cod_loinc.[LOINCCodeID] = pm.[LOINCCodeID]
		LEFT OUTER JOIN [dbo].[CodeSetProcedure] cod_proc
			ON cod_proc.[ProcedureCodeID] = pm.[ProcedureCodeID]
		WHERE (
