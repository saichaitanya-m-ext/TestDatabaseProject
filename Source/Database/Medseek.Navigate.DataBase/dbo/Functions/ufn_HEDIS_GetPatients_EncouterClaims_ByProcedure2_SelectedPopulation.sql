CREATE FUNCTION [dbo].[ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure2_SelectedPopulation]
(
...
(
	/************************************************************ INPUT PARAMETERS ************************************************************
	 ...
	 @ECTTableName = Name of the ECT Table containing Medical Codes (i.e. CPT, CPT-II, HCPCS, and ICD Procedure) to be used for determining
					 Eligible Population of Patients with qualifying Medical Claims.
	...
	WHERE (cl.[BeginServiceDate] BETWEEN (DATEADD(MM, -@Num_Months_Prior, DATEADD(YYYY, @AnchorYear_NumYearsOffset, pa.[OutputAnchorDate]))) AND
		  ...
		  (cod_proc.[ProcedureCode] IN (SELECT [ECTCode]
										...
										WHERE [ECTHedisCodeTypeCode] IN ('CPT', 'CPT-CAT-II', 'HCPCS')))
