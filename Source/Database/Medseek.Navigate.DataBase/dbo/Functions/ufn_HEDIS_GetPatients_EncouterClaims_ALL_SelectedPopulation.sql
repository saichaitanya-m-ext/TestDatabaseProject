CREATE FUNCTION [dbo].[ufn_HEDIS_GetPatients_EncouterClaims_ALL_SelectedPopulation]
(
...
(
	...
	SELECT ci.[PatientID], ci.[ClaimInfoId], ci.[ClaimNumber], ci.[EDIClaimTypeID], cod_edi.[EDIClaimTypeCode],
		   ...
		   ci.[CreatedDate] AS 'ClaimCreatedDate', ci.[LastModifiedByUserID] AS 'ClaimLastModifiedByUserID',
		   ci.[LastModifiedDate] AS 'ClaimLastModifiedDate', cl.[ClaimLineID], cl.[ProcedureCodeID],
	...
	WHERE cl.[BeginServiceDate] BETWEEN (DATEADD(MM, -@Num_Months_Prior, DATEADD(YYYY, @AnchorYear_NumYearsOffset, pa.[OutputAnchorDate]))) AND
										(DATEADD(MM, @Num_Months_After, DATEADD(YYYY, @AnchorYear_NumYearsOffset, pa.[OutputAnchorDate])))
