CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_EyeExam_Testing_Indicator_2012] (
...
BEGIN
	...
	SELECT DISTINCT [PatientID]
		,1 AS 'Count'
		,1 AS 'IsIndicator'
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_SpecialtyContionEyeExam_MP('CDC-G', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @i_ManagedPopulationID,@ReportType)
	
	UNION
	
	/* Retrieves Patients with Performed Procedures with Procedure Codes during the Year Prior to the Measurement Period. */
	SELECT DISTINCT [PatientID]
		,1 AS 'Count'
		,1 AS 'IsIndicator'
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_SpecialtyContionEyeExam_MP('CDC-G', @PopulationDefinitionID, - 1, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @i_ManagedPopulationID,@ReportType)
END
ELSE
BEGIN
	INSERT INTO #PDNR
	SELECT DISTINCT [PatientID]
		,1 AS 'Count'
		,1 AS 'IsIndicator'
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_SpecialtyContionEyeExam('CDC-G', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day,@ReportType)
	...
	SELECT DISTINCT [PatientID]
		,1 AS 'Count'
		,1 AS 'IsIndicator'
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_SpecialtyContionEyeExam('CDC-G', @PopulationDefinitionID, - 1, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day,@ReportType)
END
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_EyeExam_Testing_Indicator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
