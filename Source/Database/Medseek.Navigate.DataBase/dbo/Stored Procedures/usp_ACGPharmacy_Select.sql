/*        
------------------------------------------------------------------------------        
Procedure Name: [usp_ACGPharmacy_Select]
Description   : This Procedure used to provide ACGPharmacy details  
Created By    : NagaBabu
...
08-Mar-2011 NagaBabu Replaced #ACGPatients by ACGPharmacyProcess in Cursor
...
------------------------------------------------------------------------------        
*/  
CREATE PROCEDURE [dbo].[usp_ACGPharmacy_Select]
(  
 @i_AppUserId KEYID
)  
AS  
BEGIN TRY  
 ...
 -- Check if valid Application User ID is passed        
      IF ( @i_AppUserId IS NULL ) OR ( @i_AppUserId <= 0 )  
         ...
         END
--------------------------------------------------------------------
		DELETE FROM ACGPharmacyProcess
		...
		WHILE @@FETCH_STATUS = 0
			BEGIN
				...
				SET @vc_DynamicQSL = 'INSERT INTO	
									  ...
									  SELECT DISTINCT
										  ...
										  CONVERT(VARCHAR(10),UserDrugCodes.DateFilled,120) AS ''Rx_Fill_Date'' ,
										  CodeSetDrug.DrugCode AS ''Rx_Code'' ,
										  ''N'' AS ''Rx_Code_Type'' ,
										  UserDrugCodes.NumberOfDays AS ''Rx_Days_Supply'','''+ CAST(@i_ACGScheduleID AS VARCHAR(30)) +''',
									  ...
									  INNER JOIN UserDrugCodes
										  ON Patients.userid = UserDrugCodes.userid	
									  INNER JOIN CodeSetDrug
										  ON CodeSetDrug.DrugCodeId = UserDrugCodes.DrugCodeId ' + CAST(@vc_JoinQuerry AS VARCHAR(MAX)) + '
									  WHERE
										  UserStatusCode = ''A''
									  AND Patients.MemberNum <> ''''	  
									  AND UserDrugCodes.StatusCode = ''A''' + CAST(@vc_WhereClause AS VARCHAR(MAX)) +'
									  AND ( UserDrugCodes.DateFilled BETWEEN ''' +CONVERT(VARCHAR(10),DATEADD(MONTH,-12,@p_ParamDate)) +''' AND ''' + CONVERT(VARCHAR(10),@p_ParamDate,101) + ''')
									  ORDER BY 
										  Patients.MemberNum'	
            ...
            --SELECT DISTINCT * FROM ACGPharmacyProcess
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_ACGPharmacy_Select] TO [FE_rohit.r-ext]
    AS [dbo];
