...
INSERT INTO
    #runSQL
    SELECT
        @execSQL + schema_name(schema_id) + '.' + [name] + @space + @TO + @space + @login
    FROM
        sys.all_objects s
    WHERE
        type IN ( 'P' , 'V' , 'FN' , 'TR' , 'IF' , 'TF' , 'U' ) AND is_ms_shipped = 0
    ORDER BY
        s.type ,
        s.name
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_ExecGrantViewDefinition] TO [FE_rohit.r-ext]
    AS [dbo];
