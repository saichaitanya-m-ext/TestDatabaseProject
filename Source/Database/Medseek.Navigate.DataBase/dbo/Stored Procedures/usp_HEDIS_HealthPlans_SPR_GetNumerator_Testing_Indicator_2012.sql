CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_SPR_GetNumerator_Testing_Indicator_2012] (
	@PopulationDefinitionID INT
	,@MetricID INT
	,@Num_Months_Prior INT = 24
	,@Num_Months_After INT = 6
...
/************************************************************ INPUT PARAMETERS ************************************************************
	 ...
	 *********************************************************************************************************************************************/
/* Retrieves Patients with  during the Measurement Period. */
...
BEGIN
	...
	SELECT DISTINCT [PatientID]
		,1 AS 'Count'
		,1 AS 'IsIndicator'
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_MP('SPR-B', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @i_ManagedPopulationID, @ReportType)
END
ELSE
BEGIN
	INSERT INTO #PDNR
	SELECT DISTINCT [PatientID]
		,1 AS 'Count'
		,1 AS 'IsIndicator'
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation('SPR-B', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @ReportType)
END
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_SPR_GetNumerator_Testing_Indicator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
