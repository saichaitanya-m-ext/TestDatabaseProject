CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CMC_GetNumerator_LDL-C_Testing_Indicator_2012] (
...
BEGIN
	...
	FROM (
		...
		SELECT DISTINCT [PatientID]
			,1 AS 'Count'
			,1 AS 'IsIndicator'
		FROM dbo.ufn_HEDIS_GetPatients_LabData_SelectedPopulation_MP('CMC-D', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @i_ManagedPopulationID, @ReportType)
		) P
END
ELSE
BEGIN
	...
	FROM (
		...
		SELECT DISTINCT [PatientID]
			,1 AS 'Count'
			,1 AS 'IsIndicator'
		FROM dbo.ufn_HEDIS_GetPatients_LabData_SelectedPopulation('CMC-D', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @ReportType)
		) P
END
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CMC_GetNumerator_LDL-C_Testing_Indicator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
