CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_ASTH_GetPatients_EncouterClaims_2012]
(
...
CREATE TABLE [#ECTCodes_ASTH_Table_C]
(
...
CREATE NONCLUSTERED INDEX [IDX_ECTCodes_ASTH_Table_C] ON #ECTCodes_ASTH_Table_C
(
	[ECTCode] ASC,
...
CREATE TABLE #ASTH_Candidate_Patients
(
...
CREATE CLUSTERED INDEX [IDX_ASTH_Candidate_Patients] ON #ASTH_Candidate_Patients
(
	[PatientID] ASC,
	[ClaimInfoID] ASC,
	[ClaimLineID] ASC,
...
INSERT INTO #ECTCodes_ASTH_Table_C
SELECT [ECTCode], [ECTCodeDescription], [ECTHedisCodeTypeCode]
FROM dbo.ufn_HEDIS_GetECTCodeInfo_ByTableName('ASM-B', @ECTCodeVersion_Year, @ECTCodeStatus)



INSERT INTO #ASTH_Candidate_Patients
SELECT DISTINCT [PatientID], [ClaimInfoID], [ClaimLineID], [ProcedureCode], [RevenueCode],
				[BeginServiceDate], [EndServiceDate]

FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByDiagnosis('ASM-A', @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day,
...
(
	...
	FROM #ASTH_Candidate_Patients
	WHERE [ProcedureCode] IN (SELECT [ECTCode]
							  FROM #ECTCodes_ASTH_Table_C
							  WHERE ([ECTHedisCodeTypeCode] = 'CPT') AND
									([ECTCodeDescription] IN ('Outpatient')))
...
DROP TABLE #ECTCodes_ASTH_Table_C;
DROP TABLE #ASTH_Candidate_Patients;

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_ASTH_GetPatients_EncouterClaims_2012] TO [FE_rohit.r-ext]
    AS [dbo];
