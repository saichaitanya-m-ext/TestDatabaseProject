CREATE PROCEDURE [dbo].[usp_HEDIS_CBP_GetPatients_EncouterClaims_2012]
(
 @AnchorDate_Year int ,
...
INSERT INTO
    ...
    FROM
        ufn_HEDIS_CBP_GetPatients_EncouterClaims(@AnchorDate_Year , @AnchorDate_Month , @AnchorDate_Day , @Num_Years_Prior , @Num_Years_After , @ECTCodeStatus , @ECTCodeVersion_Year) AS pat
        ...
        ( SELECT DISTINCT
          ...
          WHERE
              [ProcedureId] IN ( SELECT
                                     cod.[ECTCode]
                                 FROM
                                     dbo.ufn_HEDIS_GetECTCodeInfo_ByTableName('CBP-B' , @ECTCodeVersion_Year , @ECTCodeStatus) AS cod
                                 WHERE
                                     ( cod.[ECTTypeCode] = 'CPT' )
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_CBP_GetPatients_EncouterClaims_2012] TO [FE_rohit.r-ext]
    AS [dbo];
