CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CHL_GetDenominator_2012] (
	...
	,@EligibleAge_MIN INT = 16
	,@EligibleAge_MAX INT = 24
...
/************************************************************ INPUT PARAMETERS ************************************************************  
  ...
  @IsPatient = Specification of the Patient Status of an Insured.  Examples = 1 (for 'Yes') or 0 (for 'No').  
...
CREATE TABLE #CHL_Insureds_Eligible ([PatientID] INT PRIMARY KEY CLUSTERED)
...
CREATE TABLE #CHL_Encounters_Eligible ([PatientID] INT PRIMARY KEY CLUSTERED)

/* Temporary Table to store 'Patient IDs' of Patients in the Eligible Population of Patients with required  
   number of Pharmacy/Rx Claims during the Measurement Period. */
CREATE TABLE #CHL_Rx_Eligible ([PatientID] INT PRIMARY KEY CLUSTERED)
...
INSERT INTO #CHL_Insureds_Eligible
...
INSERT INTO #CHL_Encounters_Eligible
EXEC dbo.usp_HEDIS_HealthPlans_CHL_GetPatients_EncounterClaims_2012 @AnchorDate_Year
	,@AnchorDate_Month
	,@AnchorDate_Day
...
/* Retrieves the 'Patient IDs' of Patients in the Eligible Population of Patients with required number  
   ...
   table. */
INSERT INTO #CHL_Rx_Eligible
EXEC dbo.usp_HEDIS_HealthPlans_CHL_GetPatients_RxClaims_2012 @AnchorDate_Year
	,@AnchorDate_Month
	,@AnchorDate_Day
	,@Num_Months_Prior_Rx
	,@Num_Months_After_Rx
...
FROM (
	...
	(
		...
		FROM #CHL_Encounters_Eligible
		
		UNION
		
		-- Retrieves Patients with Pharmacy/Rx Claims during Measurement Period.  --  
		SELECT DISTINCT [PatientID]
		FROM #CHL_Rx_Eligible
		)
	) AS p
SELECT 1
...
DROP TABLE #CHL_Insureds_Eligible;

DROP TABLE #CHL_Encounters_Eligible;

DROP TABLE #CHL_Rx_Eligible;

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CHL_GetDenominator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
