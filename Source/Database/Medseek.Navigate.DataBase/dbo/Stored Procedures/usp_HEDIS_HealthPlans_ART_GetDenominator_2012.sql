CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_ART_GetDenominator_2012]
(   @PopulationDefinitionID INT,
	@InsuranceProductTypes varchar(30) = NULL,
	@InsurancePlanTypes varchar(1000) = NULL,
	@InsuranceBenefitTypes varchar(1000) = 'Major Medical (MM), Prescription Drug (RX)',
...
CREATE TABLE #ART_Insureds_Eligible
(
...
CREATE TABLE #ART_Encounters_Eligible
(
	[PatientID] int PRIMARY KEY CLUSTERED
)
...
INSERT INTO #ART_Insureds_Eligible
EXEC dbo.usp_HEDIS_HealthPlans_GetInsureds_2012 
     @InsuranceProductTypes = @InsuranceProductTypes
    ,@InsurancePlanTypes = @InsurancePlanTypes
    ,@InsuranceBenefitTypes = @InsuranceBenefitTypes
    ,@AnchorDate_Year = @AnchorDate_Year
    ,@AnchorDate_Month = @AnchorDate_Month
    ,@AnchorDate_Day = @AnchorDate_Day
    ,@IsPrimary = @IsPrimary
    ,@NumMonths_Prior_Insured_Commercial = @NumMonths_Prior_Insured_Commercial
    ,@NumMonths_After_Insured_Commercial = @NumMonths_After_Insured_Commercial
    ,@NumMonths_Prior_Insured_Medicare = @NumMonths_Prior_Insured_Medicare
    ,@NumMonths_After_Insured_Medicare = @NumMonths_After_Insured_Medicare
    ,@NumMonths_Prior_Insured_Medicaid = @NumMonths_Prior_Insured_Medicaid
    ,@NumMonths_After_Insured_Medicaid = @NumMonths_After_Insured_Medicaid
    ,@Enrollment_AllowedNumGaps_Commercial = @Enrollment_AllowedNumGaps_Commercial
    ,@Enrollment_MAXDaysPerGap_Commercial = @Enrollment_MAXDaysPerGap_Commercial
    ,@Enrollment_AllowedNumGaps_Medicare = @Enrollment_AllowedNumGaps_Medicare
    ,@Enrollment_MAXDaysPerGap_Medicare = @Enrollment_MAXDaysPerGap_Medicare
    ,@Enrollment_AllowedNumGaps_Medicaid = @Enrollment_AllowedNumGaps_Medicaid
    ,@Enrollment_MAXDaysPerGap_Medicaid = @Enrollment_MAXDaysPerGap_Medicaid
    ,@InsuranceGroupStatus = @InsuranceGroupStatus
...
INSERT INTO #ART_Encounters_Eligible
EXEC dbo.usp_HEDIS_HealthPlans_ART_GetPatients_EncouterClaims_2012 
	 @AnchorDate_Year = @AnchorDate_Year
    ,@AnchorDate_Month = @AnchorDate_Month
    ,@AnchorDate_Day = @AnchorDate_Day
    ,@Num_Months_Prior = @Num_Months_Prior_Encounters
    ,@Num_Months_After = @Num_Months_After_Encounters
    ,@Num_Encounters_Outpatient_NonAcuteInPatient = @Num_Encounters_Outpatient_NonAcuteInPatient
    ,@Num_Encounters_AcuteInpatient_ED = @Num_Encounters_AcuteInpatient_ED
    ,@ECTCodeVersion_Year = @ECTCodeVersion_Year
    ,@ECTCodeStatus = @ECTCodeStatus


SELECT DISTINCT p.[PatientID], (CONVERT(varchar, @AnchorDate_Year) + '-' + RIGHT('0'+CAST(@AnchorDate_Month AS VARCHAR),2) + '-' + RIGHT('0'+CAST(@AnchorDate_Day AS VARCHAR),2)) AS 'AnchorDateOUT'
INTO #PDDR
FROM 
(
...
DROP TABLE #ART_Insureds_Eligible;
DROP TABLE #ART_Encounters_Eligible;

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_ART_GetDenominator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
