CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_BP_LessThan_140_90mm_Testing_Indicator_2012] (
...
CREATE TABLE #CDC_BP_140_90_Patients_ByProcedures (
...
CREATE CLUSTERED INDEX [IDX_CDC_BP_140_90_Patients_ByProcedures] ON #CDC_BP_140_90_Patients_ByProcedures (
	[PatientID] ASC
...
CREATE TABLE #CDC_BP_140_90_Patients_ByBloodPressures (
	[PatientID] INT NOT NULL
...
CREATE CLUSTERED INDEX [IDX_CDC_BP_140_90_Patients_ByBloodPressures] ON #CDC_BP_140_90_Patients_ByBloodPressures (
	[PatientID] ASC
	,[DateTaken] ASC
	)
	WITH (FILLFACTOR = 90);

DECLARE @CDC_BP_140_90_Patients_MostRecentTestDate TABLE (
	[PatientID] INT NOT NULL
...
BEGIN
	...
	INSERT INTO #CDC_BP_140_90_Patients_ByProcedures
	SELECT DISTINCT [PatientID]
		,[ProcedureCode]
		,[BeginServiceDate]
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_MP('CDC-N', @PopulationDefinitionID, 0,
	 @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month,
	  @AnchorDate_Day, @i_ManagedPopulationID,@ReportType )

	/* Stores Patients with Performed Blood Pressure readings during the Measurement Period. */
	INSERT INTO #CDC_BP_140_90_Patients_ByBloodPressures
	SELECT [PatientID]
		,[Systolic]
		,[Diastolic]
		,[ReadingTime]
	FROM dbo.ufn_GetPatients_BodyVitalSigns_BloodPressure_SelectedPopulation_MP(@PopulationDefinitionID, 0,
   ...
   was Performed for it. */
	INSERT INTO @CDC_BP_140_90_Patients_MostRecentTestDate
	SELECT p.[PatientID]
		,MAX(p.[TestDate]) AS 'TestDate'
	FROM (
		SELECT DISTINCT [PatientID]
			,[BeginServiceDate] AS 'TestDate'
		FROM #CDC_BP_140_90_Patients_ByProcedures
		
		UNION
		
		SELECT DISTINCT [PatientID]
			,[DateTaken] AS 'TestDate'
		FROM #CDC_BP_140_90_Patients_ByBloodPressures
		) AS p
	...
	FROM (
	 ...
	 Perfomed for the Patient. */
		SELECT p1.[PatientID]
		FROM @CDC_BP_140_90_Patients_MostRecentTestDate p1
		INNER JOIN #CDC_BP_140_90_Patients_ByProcedures p2
	 ...
	 Perfomed for the Patient. */
		SELECT p1.[PatientID]
		FROM @CDC_BP_140_90_Patients_MostRecentTestDate p1
		INNER JOIN #CDC_BP_140_90_Patients_ByBloodPressures p2
			ON (p2.[PatientID] = p1.[PatientID])
	...
	INNER JOIN [dbo].[PopulationDefinitionPatients] pat
	...
	INNER JOIN (
		...
		ON pdpa.PopulationDefinitionPatientID = pat.PopulationDefinitionPatientID
	WHERE pdpa.OutPutAnchorDate = (
...
BEGIN
	...
	SELECT DISTINCT [PatientID]
		,[ProcedureCode]
		,[BeginServiceDate]
	FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation('CDC-N', @PopulationDefinitionID, 0, 
	@Num_Months_Prior, 
	@Num_Months_After, 
	@ECTCodeVersion_Year, 
	@ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day,@ReportType)
...
WHEN NOT MATCHED BY SOURCE
	...
	THEN
		DELETE;
DECLARE @i_Cnt int
SELECT @i_Cnt = COUNT(*) FROM #PDNR
	
	IF @i_Cnt = 0
	BEGIN
		DELETE FROM NRPatientValue WHERE MetricID = @MetricID AND DateKey = @DateKey
	END	
...
DROP TABLE #CDC_BP_140_90_Patients_ByProcedures;

DROP TABLE #CDC_BP_140_90_Patients_ByBloodPressures;

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_BP_LessThan_140_90mm_Testing_Indicator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
