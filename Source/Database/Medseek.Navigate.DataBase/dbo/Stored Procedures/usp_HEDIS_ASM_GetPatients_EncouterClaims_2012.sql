CREATE PROCEDURE [dbo].[usp_HEDIS_ASM_GetPatients_EncouterClaims_2012]
(
 @AnchorDate_Year int ,
 ...
 @Num_Years_Prior int ,
 @Num_Years_After int ,
 @Num_Encounters_Outpatient int ,
 @Num_Encounters_ED int ,
 @Num_Encounters_AcuteInpatient int ,
...
INSERT INTO
    ...
    FROM
        ufn_HEDIS_ASM_GetPatients_EncouterClaims(@AnchorDate_Year , @AnchorDate_Month , @AnchorDate_Day , @Num_Years_Prior , @Num_Years_After , @ECTCodeStatus , @ECTCodeVersion_Year) AS pat
        ...
        ( SELECT DISTINCT
          ...
          WHERE
              [ProcedureId] IN ( SELECT
                                     cod.[ECTCode]
                                 FROM
                                     dbo.ufn_HEDIS_GetECTCodeInfo_ByTableName('ASM-B' , @ECTCodeVersion_Year , @ECTCodeStatus) AS cod
                                 WHERE
                                     ( cod.[ECTTypeCode] = 'CPT' )
                                     AND ( cod.[ECTCodeDescription] IN ( 'ED' ) ) ) ) AS cand_pat
    GROUP BY
        cand_pat.[UserId]
    HAVING
        COUNT(*) >= @Num_Encounters_ED
        ...
        ( SELECT DISTINCT
          ...
          WHERE
              [ProcedureId] IN ( SELECT
                                 ...
                                 WHERE
                                     ( cod.[ECTTypeCode] = 'CPT' )
                                     AND ( cod.[ECTCodeDescription] IN ( 'Acute inpatient' ) ) ) ) AS cand_pat
    GROUP BY
        cand_pat.[UserId]
    HAVING
        COUNT(*) >= @Num_Encounters_AcuteInpatient
INSERT INTO
    #DiabetesPatients
    SELECT
        cand_pat.[UserId]
        ...
        ( SELECT DISTINCT
          ...
          WHERE
              [ProcedureId] IN ( SELECT
                                 ...
                                 WHERE
                                     ( cod.[ECTTypeCode] = 'CPT' )
                                     AND ( cod.[ECTCodeDescription] IN ( 'Outpatient' ) ) ) ) AS cand_pat
    GROUP BY
        cand_pat.[UserId]
    HAVING
        COUNT(*) >= @Num_Encounters_Outpatient
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_ASM_GetPatients_EncouterClaims_2012] TO [FE_rohit.r-ext]
    AS [dbo];
