CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_LDL-C_Testing_Indicator_2012] (
...
BEGIN
	...
	FROM (
		SELECT DISTINCT [PatientID]
			,1 AS 'Count'
			,1 AS 'IsIndicator'
		FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_MP('CDC-H', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @i_ManagedPopulationID, @ReportType)
...
BEGIN
	...
	FROM (
		SELECT DISTINCT [PatientID]
			,1 AS 'Count'
			,1 AS 'IsIndicator'
		FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation('CDC-H', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @ReportType)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_LDL-C_Testing_Indicator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
