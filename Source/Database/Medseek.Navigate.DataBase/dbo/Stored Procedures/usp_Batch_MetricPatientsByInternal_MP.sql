...
/*  
---------------------------------------------------------------------------------------  
Procedure Name: [dbo].[usp_Batch_MetricPatientsByInternal_MP]  
...
----------------------------------------------------------------------------------------  
*/
CREATE PROCEDURE [dbo].[usp_Batch_MetricPatientsByInternal_MP] --1,@v_DateKey
	(
	@i_AppUserId KEYID
	,@v_DateKey VARCHAR(8)
	,@i_MetricID INT
	...
	AS (
		...
		INNER JOIN Metric m WITH (NOLOCK)
			ON m.MetricId = pdc.MetricID
				AND m.DenominatorID = pdp.PopulationDefinitionID---dr
		...
		WHERE pdc.MetricID IS NOT NULL
			...
			AND m.ManagedPopulationID = pp.ProgramID
			AND mrc.Datekey = @v_DateKey
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_Batch_MetricPatientsByInternal_MP] TO [FE_rohit.r-ext]
    AS [dbo];
