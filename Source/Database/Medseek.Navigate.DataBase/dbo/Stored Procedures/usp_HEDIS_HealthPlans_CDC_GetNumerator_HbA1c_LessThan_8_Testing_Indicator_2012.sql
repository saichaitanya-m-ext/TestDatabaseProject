CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_HbA1c_LessThan_8_Testing_Indicator_2012] (
...
CREATE TABLE #CDC_HbA1c_8_Patients_ByProcedures (
...
CREATE CLUSTERED INDEX [IDX_CDC_HbA1c_8_Patients_ByProcedures] ON #CDC_HbA1c_8_Patients_ByProcedures (
	[PatientID] ASC
...
CREATE TABLE #CDC_HbA1c_8_Patients_ByLabTest (
	[PatientID] INT NOT NULL
...
CREATE CLUSTERED INDEX [IDX_CDC_HbA1c_8_Patients_ByLabTest] ON #CDC_HbA1c_8_Patients_ByLabTest (
	[PatientID] ASC
	,[DateTaken] ASC
	)
	WITH (FILLFACTOR = 90);

DECLARE @CDC_HbA1c_8_Patients_MostRecentTestDate TABLE (
	[PatientID] INT NOT NULL
...
CREATE TABLE #PDNR (
	...
	BEGIN
		...
		INSERT INTO #CDC_HbA1c_8_Patients_ByProcedures
		SELECT DISTINCT [PatientID]
			,[ProcedureCode]
			,[BeginServiceDate]
		FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation_MP('CDC-Q', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @i_ManagedPopulationID, @ReportType)

		/* Stores Patients with Performed Lab Tests with LOINC Codes during the Measurement Period. */
		INSERT INTO #CDC_HbA1c_8_Patients_ByLabTest
		SELECT [PatientID]
			,[MeasureValueNumeric]
			,[DateTaken]
   ...
   Performed for it. */
		INSERT INTO @CDC_HbA1c_8_Patients_MostRecentTestDate
		SELECT p.[PatientID]
			,MAX(p.[TestDate]) AS 'TestDate'
		FROM (
			SELECT DISTINCT [PatientID]
				,[BeginServiceDate] AS 'TestDate'
			FROM #CDC_HbA1c_8_Patients_ByProcedures
			
			UNION
			
			SELECT DISTINCT [PatientID]
				,[DateTaken] AS 'TestDate'
			FROM #CDC_HbA1c_8_Patients_ByLabTest
			) AS p
	 ...
	 Perfomed for the Patient. */
			SELECT p1.[PatientID]
			FROM @CDC_HbA1c_8_Patients_MostRecentTestDate p1
			INNER JOIN #CDC_HbA1c_8_Patients_ByProcedures p2 ON (p2.[PatientID] = p1.[PatientID])
				AND (p2.[BeginServiceDate] = p1.[TestDate])
			WHERE p2.[ProcedureCode] = '3044F'
	 ...
	 Perfomed for the Patient. */
			SELECT p1.[PatientID]
			FROM @CDC_HbA1c_8_Patients_MostRecentTestDate p1
			INNER JOIN #CDC_HbA1c_8_Patients_ByLabTest p2 ON (p2.[PatientID] = p1.[PatientID])
				AND (p2.[DateTaken] = p1.[TestDate])
			WHERE (p2.[MeasureValueNumeric] < 8)
	...
	BEGIN
		...
		SELECT DISTINCT [PatientID]
			,[ProcedureCode]
			,[BeginServiceDate]
		FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByProcedure_SelectedPopulation('CDC-Q', @PopulationDefinitionID, 0, @Num_Months_Prior, @Num_Months_After, @ECTCodeVersion_Year, @ECTCodeStatus, @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day, @ReportType)
...
DROP TABLE #CDC_HbA1c_8_Patients_ByProcedures;

DROP TABLE #CDC_HbA1c_8_Patients_ByLabTest;

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CDC_GetNumerator_HbA1c_LessThan_8_Testing_Indicator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
