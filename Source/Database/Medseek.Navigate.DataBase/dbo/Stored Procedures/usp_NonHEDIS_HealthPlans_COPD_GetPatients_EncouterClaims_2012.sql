...
CREATE PROCEDURE [dbo].[usp_NonHEDIS_HealthPlans_COPD_GetPatients_EncouterClaims_2012]    
(    
...
CREATE TABLE [#ECTCodes_SPR_Table_C]    
(    
...
CREATE NONCLUSTERED INDEX [IDX_ECTCodes_SPR_Table_C] ON #ECTCodes_SPR_Table_C    
(    
 [ECTCode] ASC,    
...
CREATE TABLE #SPR_Candidate_Patients    
(    
...
CREATE CLUSTERED INDEX [IDX_SPR_Candidate_Patients] ON #SPR_Candidate_Patients    
(    
 [PatientID] ASC,    
 [ClaimInfoID] ASC,    
 [ClaimLineID] ASC,    
...
INSERT INTO #ECTCodes_SPR_Table_C    
SELECT [ECTCode], [ECTCodeDescription], ECTHedisCodeTypeCode    
FROM dbo.ufn_HEDIS_GetECTCodeInfo_ByTableName('SPR-C', @ECTCodeVersion_Year, @ECTCodeStatus)    
...
INSERT INTO #SPR_Candidate_Patients    
SELECT DISTINCT [PatientID], [ClaimInfoID], [ClaimLineID], [ProcedureCode], [RevenueCode],    
    [BeginServiceDate], [EndServiceDate]    
    
FROM dbo.ufn_HEDIS_GetPatients_EncouterClaims_ByDiagnosis2('SPR-A', @AnchorDate_Year, @AnchorDate_Month, @AnchorDate_Day,    
...
SELECT DISTINCT [PatientID], [ClaimInfoID], [BeginServiceDate]    
    FROM #SPR_Candidate_Patients    
    WHERE [ProcedureCode] IN (SELECT [ECTCode]    
         FROM #ECTCodes_SPR_Table_C    
         WHERE ([ECTTypeCode] = 'CPT') AND    
...
DROP TABLE #ECTCodes_SPR_Table_C;    
DROP TABLE #SPR_Candidate_Patients;  
  

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_NonHEDIS_HealthPlans_COPD_GetPatients_EncouterClaims_2012] TO [FE_rohit.r-ext]
    AS [dbo];
