/*
----------------------------------------------------------------------------------
Procedure Name: [usp_Patients_Search] 10926,@v_SecurityRoleName = 'Care Manager',@i_ChildProviderID = 41558 ,@i_HealthPlanID = 1,@i_ProductID = 1,@v_LastName = 'Stephen'
...
*/
CREATE PROCEDURE [dbo].[usp_Patients_Search] @i_AppUserId KEYID
,@i_UserID INT = NULL
,@i_StartIndex INT = 1
,@i_EndIndex INT = 500
...
SET @v_PatientSQL = 'INSERT INTO #User (Userid,SecurityName)
...
SELECT DISTINCT TOP ( ' + CONVERT(VARCHAR(10), @i_EndIndex) + ') Patients.PatientID,''Patient'',pp.Providerid FROM Patients WITH(NOLOCK)
INNER JOIN PatientPCP pp WITH(NOLOCK)
ON pp.PatientID = Patients.PatientID
INNER JOIN ProviderHierarchyDetail phd
ON pp.ProviderID = phd.childproviderid '
...
SET @v_PatientSQL = 'INSERT INTO #User
SELECT DISTINCT TOP ( ' + CONVERT(VARCHAR(10), @i_EndIndex) + ') Patients.PatientID,''Patient'',pp1.Providerid FROM Patients WITH(NOLOCK)
INNER JOIN PatientPCP pp1 WITH(NOLOCK)
ON pp1.PatientID = Patients.PatientID
INNER JOIN ProviderHierarchyDetail phd1
ON pp1.ProviderID = phd1.childproviderid '
...
'
SET @v_PatientSQL = 'INSERT INTO #User (Userid,SecurityName,PCPID)
SELECT DISTINCT TOP ( ' + CONVERT(VARCHAR(10), @i_EndIndex) + ') Patients.PatientID,''Patient'',pp.Providerid FROM Patients WITH(NOLOCK)
INNER JOIN PatientPCP pp WITH(NOLOCK)
ON pp.PatientID = Patients.PatientID
INNER JOIN PatientInsurance i WITH (NOLOCK)
...
SET @v_PatientSQL = 'INSERT INTO #User (Userid,SecurityName,PCPID)
SELECT DISTINCT TOP ( ' + CONVERT(VARCHAR(10), @i_EndIndex) + ') Patients.PatientID,''Patient'',pcp.Providerid FROM Patients WITH(NOLOCK)
INNER JOIN PatientPCP pcp WITH(NOLOCK)
ON pcp.PatientID = Patients.PatientID
INNER JOIN PatientProgram pp WITH ( NOLOCK )
ON Patients.PatientID = pp.PatientID
...
,dbo.ufn_GetProviderName(u.PCPID) AS PCPName
,DateOfBirth
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_Patients_Search] TO [FE_rohit.r-ext]
    AS [dbo];
