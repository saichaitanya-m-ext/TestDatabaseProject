CREATE PROCEDURE [dbo].[usp_HEDIS_HealthPlans_CMC_GetDenominator_2012] (
...
CREATE TABLE #CMC_Insureds_Eligible_CurrentYear ([PatientID] INT PRIMARY KEY CLUSTERED)
...
CREATE TABLE #CMC_Insureds_Eligible_PriorYear ([PatientID] INT PRIMARY KEY CLUSTERED)
...
CREATE TABLE #CMC_Encounters_Eligible ([PatientID] INT PRIMARY KEY CLUSTERED)
...
INSERT INTO #CMC_Insureds_Eligible_CurrentYear
...
INSERT INTO #CMC_Insureds_Eligible_PriorYear
EXEC dbo.usp_HEDIS_HealthPlans_GetInsureds_2012 @InsuranceProductTypes
	,@InsurancePlanTypes
	,@InsuranceBenefitTypes
...
/* Retrieves the 'Patient IDs' of Patients in the Eligible Population of Patients with required number  
   ...
   print 1
INSERT INTO #CMC_Encounters_Eligible
EXEC dbo.usp_HEDIS_HealthPlans_CMC_GetPatients_EncounterClaims_2012 @AnchorDate_Year
	,@AnchorDate_Month
	,@AnchorDate_Day
...
--(CONVERT(VARCHAR, @AnchorDate_Year)  + CASE WHEN LEN(@AnchorDate_Month) =1 THEN  '0'+ CONVERT(VARCHAR, @AnchorDate_Month) ELSE CONVERT(VARCHAR, @AnchorDate_Month) END + CASE WHEN LEN(@AnchorDate_Day) = 1 THEN '0'+CONVERT(VARCHAR, @AnchorDate_Day) ELSE CONVERT(VARCHAR, @AnchorDate_Day) END)
...
FROM (
	...
	(
		...
		SELECT [PatientID]
		FROM #CMC_Insureds_Eligible_CurrentYear
		...
		SELECT [PatientID]
		FROM #CMC_Insureds_Eligible_PriorYear
		)
...
DROP TABLE #CMC_Insureds_Eligible_CurrentYear;

DROP TABLE #CMC_Insureds_Eligible_PriorYear;

DROP TABLE #CMC_Encounters_Eligible;

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_HealthPlans_CMC_GetDenominator_2012] TO [FE_rohit.r-ext]
    AS [dbo];
