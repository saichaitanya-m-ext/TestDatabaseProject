CREATE PROCEDURE [dbo].[usp_HEDIS_ART_GetPatients_EncouterClaims_2012]
(
 @AnchorDate_Year int ,
 ...
 @Num_Years_Prior int ,
 @Num_Years_After int ,
 @Num_Encounters_Outpatient_NonAcuteInPatient int ,
 @Num_Encounters_AcuteInpatient_ED int ,
...
INSERT INTO
    ...
    FROM
        ufn_HEDIS_CDC_GetPatients_EncouterClaims(@AnchorDate_Year , @AnchorDate_Month , @AnchorDate_Day , @Num_Years_Prior , @Num_Years_After , @ECTCodeStatus , @ECTCodeVersion_Year) AS pat
        ...
        ( SELECT DISTINCT
          ...
          WHERE
              [ProcedureId] IN ( SELECT
                                 ...
                                 WHERE
                                     ( cod.[ECTTypeCode] = 'CPT' )
                                     AND ( cod.[ECTCodeDescription] IN ( 'Outpatient' , 'Nonacute inpatient' ) ) ) ) AS cand_pat
    GROUP BY
        cand_pat.[UserId]
    HAVING
        ...
        ( SELECT DISTINCT
          ...
          WHERE
              [ProcedureId] IN ( SELECT
                                 ...
                                 WHERE
                                     ( cod.[ECTTypeCode] = 'CPT' )
                                     AND ( cod.[ECTCodeDescription] IN ( 'Acute inpatient' , 'ED' ) ) ) ) AS cand_pat
    GROUP BY
        cand_pat.[UserId]
    HAVING
        COUNT(*) >= @Num_Encounters_AcuteInpatient_ED
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[usp_HEDIS_ART_GetPatients_EncouterClaims_2012] TO [FE_rohit.r-ext]
    AS [dbo];
